...
export class RssAdapter extends BaseProviderAdapter {
  ...
  async fetchOnce(): Promise<SourceRecord[]> {
    const feeds = this.definition.endpoints?.filter((endpoint) => endpoint.enabled !== false) ?? [];
    const settled = await Promise.allSettled(
      feeds.map(async (feed) => {
        const response = await fetch(feed.url, { next: { revalidate: 180 } });
        if (!response.ok) {
          throw new Error(`RSS fetch failed for ${feed.description ?? feed.key}`);
        }
  ...
  async normalize(raw: unknown): Promise<SourceRecord[]> {
    const payload = raw as { feed: ProviderEndpoint; parsed: any };
    ...
    return normalizedItems.filter(Boolean).slice(0, 12).map((item: any, index: number) => {
      ...
      return this.baseRecord({
        id: `${payload.feed.key}-${index}-${rawHash.slice(0, 12)}`,
        ...
        entities: [],
        locations: [],
        tags: ["news", payload.feed.key.toLowerCase().replace(/\s+/g, "-")],
        rawHash,
        rights: this.defaultRights("Preserve publisher attribution and original links."),
        metadata: {
          feedId: payload.feed.key,
          feedName: payload.feed.description ?? payload.feed.key
        },
        rawPayload: item
      });
    });
